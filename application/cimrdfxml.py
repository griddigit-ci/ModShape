...
__all__ = ["create_parser", "BagID", "ElementHandler", "RDFXMLHandler", "CIMRDFXMLParser"]
...
class RDFXMLHandler(handler.ContentHandler):
    def __init__(self, store: Graph,datamapping):
        self.store = store
...
def create_parser(target: InputSource, store: Graph, datamapping) -> xmlreader.XMLReader:
    parser = make_parser()
    try:
    ...
    rdfxml = RDFXMLHandler(store,datamapping)
...
class CIMRDFXMLParser(Parser):
    def __init__(self):
        super().__init__()

    def parse(self, source: InputSource, sink: Graph, **args: Any) -> None:
        datamapping = args.get('datatype_mapping')
        self._parser = create_parser(source, sink,datamapping)
